 ...
 */
public class BlastnMapper extends Mapper<Text, BytesWritable, IntWritable, Text>{ // Alignment> {
	...
	private byte[] seqbytes = new byte[Fasta2SequenceFileConverter.bpAmountPerLineDefault+
	...
	ArrayList<GapFreeExtension> filteredExt = new ArrayList<GapFreeExtension>();
	/**
	...
	public void setup(Context context) throws IOException {
		...
		this.mismatch= conf.getInt("blastn.argument.mistmatchScore", -3);
		this.gap = conf.getInt("blastn.argument.gapPenalty", -5);
		
		this.twoHitDistanceA = conf.getInt("blastn.argument.twoHitDistanceA", 40);

		this.ungappedScoreThreshold = conf.getInt("blastn.argument.ungappedScoreThreshold", 100*match);
		this.gappedScoreThreshold = conf.getInt("blastn.argument.gappedScoreThreshold", 200*match);
		
		this.ungappedXDrop = conf.getInt("blastn.argument.ungappedX", 10*match);
		this.gappedXDrop = conf.getInt("balstn.argument.gappedX", 10*match);
	 ...
	 */
	@Override
	public void map(Text key, BytesWritable val, Context context) throws IOException, InterruptedException{
		// extract bases in val, into a byte array,///////////// 
		// in which each byte contains one bp(encoded in 4 bits) on the right half byte
		...
		if(isLastRecordOfSeq){
			twoHitList = scanner.scan(seqbytes, 0, seqSplitLen+overlapLen,
					twoHitDistanceA);
		}else{
			twoHitList = scanner.scan(seqbytes, 0, seqSplitLen+wordLen, twoHitDistanceA);
		}
		//TODO:
...
//		System.out.println("seqID:newone");
		...
		for(TwoHit twoHit: twoHitList){
			...
			// if consecutive hits
			if((twoHit.getSecondIndexInDB_Sequence()==preTwoHitSubjectIndex
					&&twoHit.getSecondIndexInQuery()==preTwoHitQueryIndex)
					|| (twoHit.getSecondIndexInDB_Sequence()+1 == preTwoHitSubjectIndex 
					&& twoHit.getSecondIndexInQuery()+1 == preTwoHitQueryIndex)
					|| (twoHit.getSecondIndexInDB_Sequence()-1 == preTwoHitSubjectIndex 
					&& twoHit.getSecondIndexInQuery()-1 == preTwoHitQueryIndex)){
				preTwoHitSubjectIndex=twoHit.getSecondIndexInDB_Sequence();
			...
			for(GapFreeExtension ext: filteredExt){
				//if on the same diagonal and in the range of ext's extension
				int dist1 = twoHit.getSecondIndexInDB_Sequence()-ext.twoHit.getSecondIndexInDB_Sequence(); 
				int dist2 = twoHit.getSecondIndexInQuery()-ext.twoHit.getSecondIndexInQuery(); 
				if(dist1 == dist2 && dist1 >= ext.leftEnd && dist1 <= ext.rightEnd){
					isRepeatHit = true;
					break;
				}
			}
			...
			//gap free extending
			gapFreeExt = ungappedExtend(queryBytes, 0, queryBytes.length,
		...
		for(int i = 0; i<10 && i<filteredExt.size(); i++){
			...
			dpExtender.setParameters(
					...
					filteredExt.get(i).twoHit.getSecondIndexInDB_Sequence(),
			...
			try {
				if(dpExtender.extend(gappedScoreThreshold)){
					...
					outValueAlign.setSubjectSeqStartOffset(
							seqStartOffset+outValueAlign.getSubjectSeqStartOffset());
	 ...
	 * @return
	 */
	static GapFreeExtension ungappedExtend(byte[] querySeq, int queryBegin, int queryEnd, 
			byte[] dbSeq, int dbBegin, int dbEnd, 
			TwoHit twoHit,
		...
		int forwardIndex=0;
		for(; ;forwardIndex++){
			if(forwardIndex+queryHitIndex>=queryEnd || forwardIndex+dbHitIndex >= dbEnd){
				break;
			}
			//match
			if(querySeq[queryHitIndex+forwardIndex]==dbSeq[dbHitIndex+forwardIndex]){
		...
		int backwardIndex=0;
		for(; ;backwardIndex++){
			if(queryHitIndex-1-backwardIndex<queryBegin|| dbHitIndex-1-backwardIndex < dbBegin){
				break;
			}
			
			if(querySeq[queryHitIndex-1-backwardIndex]==dbSeq[dbHitIndex-1-backwardIndex]){
...
class GapFreeExtension implements Comparable{
	public int score;
	public TwoHit twoHit;
	public int leftEnd;
	public int rightEnd;
	public GapFreeExtension(int score, TwoHit hit, int leftEnd, int rightEnd ){
		this.score = score;
		this.twoHit = hit;
	...
	public int compareTo(Object other) {
		...
		return -(this.score - ((GapFreeExtension)other).score);
	}
}