 ...
 */
public class HFileOutputFormat extends FileOutputFormat<ImmutableBytesPairWritable, KeyValue> {
    ...
    public RecordWriter<ImmutableBytesPairWritable, KeyValue> getRecordWriter(final TaskAttemptContext context)
        ...
        return new RecordWriter<ImmutableBytesPairWritable, KeyValue>() {
            ...
            public void write(ImmutableBytesPairWritable pair, KeyValue kv)
                    throws IOException {
                if(pair==null){
                    rollWriters();
                    return;
                }
                byte[] tableName = pair.getFirst();
                ImmutableBytesWritable row = new ImmutableBytesWritable(pair.getSecond());
                ...
                byte[] tableNameAndFamily = ArrayUtils.addAll(tableName, family);
                WriterLength wl = this.writers.get(tableNameAndFamily);

                // If this is a new column family, verify that the directory exists
                if (wl == null) {
                    fs.mkdirs(new Path(new Path(outputdir, Bytes.toString(tableName)), Bytes.toString(family)));
                }
                ...
                if (wl == null || wl.writer == null) {
                    wl = getNewWriter(tableNameAndFamily, tableName, family, conf);
                }
             ...
             * @throws IOException
             */
            private WriterLength getNewWriter(byte[] tableNameAndFamily, byte[] tableName, byte[] family, Configuration conf)
                    throws IOException {
                WriterLength wl = new WriterLength();
                Path familydir = new Path(new Path(outputdir, Bytes.toString(tableName)), Bytes.toString(family));
                ...
                wl.writer = HFile.getWriterFactoryNoCache(conf)
                        ...
                        .create();
                this.writers.put(tableNameAndFamily, wl);
                return wl;
            }
     ...
     * @param allStartKeys   the region start keys
     */
    private static void writePartitions(Configuration conf, Path partitionsPath,
                                        Map<HTable, List<ImmutableBytesWritable>> allStartKeys) throws IOException {
        for (List<ImmutableBytesWritable> startKeys : allStartKeys.values()) {
        ...
        SequenceFile.Writer writer = SequenceFile.createWriter(fs,
                conf, partitionsPath, ImmutableBytesPairWritable.class, NullWritable.class);

        try {
            List<ImmutableBytesPairWritable> list = new ArrayList<ImmutableBytesPairWritable>();
            for (Map.Entry<HTable, List<ImmutableBytesWritable>> entry : allStartKeys.entrySet()) {
                for (ImmutableBytesWritable startKey : entry.getValue()) {
                    ImmutableBytesPairWritable key = new ImmutableBytesPairWritable(entry.getKey().getTableName(), startKey.copyBytes());
                    list.add(key);

                }
            }
            Collections.sort(list,new Comparator<ImmutableBytesPairWritable>() {
                @Override
                public int compare(ImmutableBytesPairWritable o1, ImmutableBytesPairWritable o2) {
                    return o1.compareTo(o2);
                }
            });
            for (ImmutableBytesPairWritable key : list) {
     ...
     */
    public static void configureIncrementalLoad(Job job, List<HTable> tables)
            throws IOException {
        ...
        Map<HTable, List<ImmutableBytesWritable>> allStartKeys = new HashMap<HTable, List<ImmutableBytesWritable>>();
        for (HTable table : tables) {
            LOG.info("Looking up current regions for table " + table);
            List<ImmutableBytesWritable> startKeys = getRegionStartKeys(table);
            allStartKeys.put(table, startKeys);
            int size = startKeys.size();
            LOG.info("Configuring " + size + " reduce partitions " +
                    "to match current region count");
            totalRegionCount += size;
        }
//        job.setNumReduceTasks(totalRegionCount - tables.size() + 1);
        job.setNumReduceTasks(totalRegionCount + 1);
