    ...
    while (iter.hasNext()) {
      ...
      while (iter.hasNext() && curSize < threshold) {
        ...
        for (List<KeyValue> kvs : p.getFamilyMap().values()) {
          for (KeyValue kv : kvs) {
//        	KeyValue newKv = new KeyValue(newRowkey.getBytes(), kv.getFamily(), 
//        			kv.getQualifier(), kv.getMemstoreTS(), kv.getValue());
            map.add(kv);
            curSize += kv.getLength();
          }
        }
      }
//      System.out.println("rowkey : " + new String(row.get()) + ", record size : " + record_index);
